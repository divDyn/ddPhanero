---
title: "Phanerozoic-scale global marine biodiversity analysis with the R package divDyn v0.7"
author: "Adam T. Kocsis, John Alroy, Carl J. Reddin, Wolfgang Kiessling"
date: '`r Sys.Date()`'
output: pdf_document
vignette: >
  %\VignetteIndexEntry{Phanerozoic-scale global marine biodiversity analyses with the R package divDyn v0.7}
...
```
and are referenced directly from the vignette. Along with the vignette, the data will be updated with every major release. The analytical script presented here is also available as a regular .R file (scripts/2019-01-03_marineAnimals_ddPhanero.R).
...
To ensure the exact reproducibility of the material presented here, the raw dataset that was used to calculate the results (downloaded on January 3, 2019) is available for download and experimentation. If you do not wish to carry out a PaleoDB download yourself, you can use our file. 

```{r dat2, echo= TRUE}
load(url(
"https://github.com/adamkocsis/ddPhanero/raw/master/data/PaleoDB/2019-01-03_paleoDB.RData"
))
```
...
It will take a considerable amount of time (30+ minutes) to download this set, and the resulting file is very large (ca. 1.4 GB). Nevertheless, it can be stored effectively by omitting some variables and saving it as a binary .RData format. You can load the file (saved as "allData_2019-01-03.csv" in this case) using the ``read.csv()`` function.

```{r dat1, echo= TRUE, eval=FALSE}
# assuming that the file is in your current working directory
dat <- read.csv("allData_2019-01-03.csv", header=TRUE, stringsAsFactors=FALSE)
```
...
Using a complete download of collections from Fossilworks, we compiled a table that resolves these 'interval' entries to the timescales of our interest, which can be viewed in the ``stratkeys`` object, which can be invoked by ``data(stratkeys)``.  The rows in this table were then transformed to ``list`` type entries that can be used by the ``categorize()`` function. Similarly to the environmental variables, this can be found in the ``keys`` object. 
...
source(
"https://github.com/divDyn/ddPhanero/raw/master/scripts/strat/2018-08-31/ordProcess.R")
```
...
# 2. CR
crBins <- subsample(dat, bin="bin", tax="clgen", coll="collection_no", q=4800, iter=500, 
  duplicates=FALSE)
# 3. SQS
sqsBins <- subsample(dat, bin="bin", tax="clgen", coll="collection_no", q=0.7, iter=500, 
...
```
https://github.com/adamkocsis/ddPhanero/raw/master/export/detrending.pdf
```
...
Work on the package was funded by the Deutsche Forschungsgemeinschaft (Ko 5382/1-1, Ko 5382/1-2 and Ki 806/16-1) and is part of the Research Unit TERSANE (FO 2332). We thank all contributors of the Paleobiology Database, especially M. Clapham, A. Hendy, M. Carrano, A. Miller, M. Uhen, M. Aberhan B. Kröger, P. Wagner, M. Patzkowsky, M. Foote and J. Pálfy. We are also thankful to Na Lin, for assigning the Cambrian collections to the stages. 
